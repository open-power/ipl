...
static bool set_or_clear_state(struct pdbg_target* target, bool do_set)
{
    ...
    if (!pdbg_target_set_attribute_packed(target, "ATTR_HWAS_STATE", "41", 1,
                                          buf))
    {
...
static void update_hwas_state(bool is_coldboot)
{
    ...
    if (records.size())
    {
        ...
        for (const auto& elem : records)
        {
            ...
            err = pdbg_target_traverse(NULL, update_hwas_state_callback,
                                       &targetinfo);
            if ((err == 0) || (err == 2))
                ipl_log(IPL_ERROR,
...
{
    std::array<const char*, 8> mProcChild = {"core", "pauc",    "pau", "iohs",
                                             "mc",   "chiplet", "pec", "fc"};
    ...
    pdbg_for_each_class_target("proc", proc)
    {
        ...
        {
            ipl_log(IPL_ERROR, "Failed to set HWAS state of proc %d\n",
    ...
    pdbg_for_each_class_target("proc", proc)
    {
        ...
        if (ipl_is_master_proc(proc))
        {
            if (ipl_type() == IPL_TYPE_MPIPL)
            {
                ...
                {
                    ipl_log(IPL_INFO,
                            "Running sbe_mpipl_continue on processor %d\n",
                            pdbg_target_index(proc));
                    ret = sbe_mpipl_continue(pib);
                    if (ret != 0)
                    {
                        ipl_log(IPL_ERROR, "Continue MPIPL failed, ret=%d\n",
                                ret);
                    }
