...
static int ipl_fabric_io_dccal(void)
{
	...
	pdbg_for_each_class_target("pib", pib)
	{
		...
		pdbg_for_each_target("xbus", pib, target)
		{
			...
			FAPI_ATTR_GET(fapi2::ATTR_IO_XBUS_MASTER_MODE, target,
				      master_mode);
			if (master_mode)
				master_xbus = target;
			else
...
static int ipl_proc_smp_link_layer(void)
{
	...
	pdbg_for_each_class_target("pib", pib) p9_smp_link_layer(pib, 1, 0);
...
static int ipl_host_fbc_eff_config_aggregate(void)
{
	...
	pdbg_for_each_class_target("pib", pib) p9_fbc_eff_config_aggregate(pib);
