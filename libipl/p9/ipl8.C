...
static int ipl_proc_check_slave_sbe_seeprom_complete(void)
{
	...
	pdbg_for_each_class_target("pib", pib)
	{
		...
		/* This procedure is listed in istep list but is a noop in the
		 * ekb */
...
static int ipl_proc_attr_update(void)
{
	...
	pdbg_for_each_class_target("pib", pib) p9_attr_update(pib);
...
static int ipl_proc_chiplet_scominit(void)
{
	...
	pdbg_for_each_class_target("pib", pib) p9_chiplet_fabric_scominit(pib);
...
static int ipl_proc_xbus_enable_ridi(void)
{
	...
	pdbg_for_each_class_target("pib", pib) p9_xbus_enable_ridi(pib);
